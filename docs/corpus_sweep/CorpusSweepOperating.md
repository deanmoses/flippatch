...
- `evidence` — optional source refs (defaults to the model's own `ipdb:` note). Any scheme `quotes.sources`'s `free_text_for` resolves works, so tilt.it / flippers.be web-cache pages plug in per model.
...
| `fill` — no matching catalog edge; verbatim quote; a unique resolution (machine target) or a text label; quote **supports** the claim (ai_lint's quote-supports-claim rule, the same standard `make verify-quote-support` applies) | `set-but-unsupported` — a catalog edge no judged claim accounts for (its target/type/license the note doesn't support — the wrong-seeded case)                               |
