...
| `joker-manilamatic`                      | tilt.it (Italian)  | `https://www.tilt.it/flipper_pinball/ipdb/manilamatic`                                                       | terse maker-index, Italian                                        | bootleg lineage in one list line (_"Joker (copia del Gottlieb's 'Monte Carlo')"_); the lineage slice on non-English text; siblings that entity-discovery will later surface                                                                                                                                             |
...
| `grand-canyon`           | gameplay: passive bumpers (12), kick-out hole; **lineage** `conversion` / `converted_from` → Exhibit's 'Double Play' + `conversion` tag                                                                            | —                                                                                                                                                                                                |
...
| `wizard`                 | gameplay (rich); **reward** `add-a-ball` + `novelty`; **lineage** `conversion` / `converted_from` → Gottlieb's 1977 'Team One' + `conversion` tag; tech-gen `electromechanical` (drop targets, wedge head, no CPU) | inferring solid-state from the 2022 date (the prose describes an EM machine)                                                                                                                     |
...
| bootleg (lineage)                 | `bootleg` tag + `bootleg_of` → Gottlieb's Monte Carlo                             | "Joker (copia del Gottlieb's 'Monte Carlo')"      |
...
**Traps (must NOT attach to Joker):** themes `Tag Team` / `Arena` (sibling machines Out Law / Defender's copied games) and `Monte Carlo` (the _copied_ game's name, not a theme); credits `Tito Bresca` (designed sibling _Lucky Man_) and `Antonio Manili` (the company operator); lineage `bootleg` for Out Law→Tag Team and Defender→Arena (sibling bootlegs). The baseline run surfaced all of these against Joker.
