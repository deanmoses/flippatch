...
- [ ] **game_format** — one of: `pinball` / `bagatelle` / `shuffle` / `pitch-and-bat` / `flasher-type-gambling-console` / `video-game` / `gun-game` / `miscellaneous`. (A non-pinball machine silently defaulting to `pinball` is a classic error — decide it explicitly.)
