...
- [ ] **tag(s)** — labels that don't fit a structured field; a bounded set of independent binary labels, so decide each present/absent. (The `bootleg`, `remake`, and `conversion-kit` tags each pair with a lineage relationship — handle them as the lineage questions in A2, not here.)
...
Each is **one question** that can produce a tag, a relationship, or both — and the tag often stands alone, since a source frequently says a machine _is_ a bootleg/remake/conversion without naming the specific game it derives from. Set the relationship only when the source names the source machine; set the tag whenever the kind is established. Each relationship claim needs its own cite (the line stating the lineage). Watch the polarity trap: _"its companion model is a widebody"_ does **not** make _this_ model a widebody.

- [ ] **bootleg?** — an unauthorized, built-from-scratch copy of another maker's game (common among mid-century Italian/Spanish makers). If yes: set the **`bootleg` tag**, and **`bootleg_of`** → the copied game when the source names it. Keeps a normal `produced` status; `bootleg_of` routinely points across Titles.
...
- [ ] **conversion?** — reuses another machine's physical cabinet with a new playfield/theme. If yes: set **`converted_from`** → the donor machine when named, and add the **`conversion-kit` tag** when it's sold as a kit of parts to install rather than a complete machine.
