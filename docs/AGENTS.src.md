...
The files under `external_data_sources/` are internals, split for editing: `identity.sql` holds both sources' decodes and the one shared model-matching ladder, classification and replay (run once over a unified listing shape — IPDB rows simply carry no group leg); `assertions.sql` lands both sources' classification assertions (IPDB specialties, OPDB vocabulary) on the catalog through one resolution lookup and one carriage rule; and `ipdb.sql` / `opdb.sql` hold what each source alone has (corporate entities and credits and specialty policy; titles and manufacturers and the IPDB cross-check) plus their findings. A session editing one of them can read just that file (`PREFIX=identity`, `PREFIX=assertions`, or the source name, e.g. `PREFIX=opdb`) — that path loads the live SQL, uncached, which is the point when editing it.
...
- **Trusting the matches** — the known-good replay (in `identity.sql`) covers up the link on every already-linked listing and asks the ladder to re-derive it, so matcher accuracy is a number the run prints rather than a matter of belief. Read `replay_answers_would_link` beside `REPLAY_DISAGREEMENTS_would_link` in the summary: the second is answers the layer would have published as a link that the record contradicts, and it is the count that must stay at zero. A `replay_disagreements_to_human` row would have reached a person anyway (`possible_duplicate`), so it is cheaper. The per-row evidence is `opdb_known_good_replay` / `ipdb_known_good_replay`. Because it must test the shipping matcher rather than a copy, the candidate and resolution tables are deliberately unscoped and every consumer filters at its own site — don't re-scope them.
- **Retiring a finding permanently** — every recorded human judgment is one row in `bridge.sql`'s adjudications relation, appended with a note (and a date, for new rows) under the narrowest scope that carries the decision: `finding` dismisses one finding, `maker-pair` clears an adjudicated maker pairing everywhere at once (including the matcher's contested-pair guard), `vocabulary-value` settles an absent value as permanently not ours to mint. Browse them all with per-scope staleness in `external_data_source_adjudications`; the per-scope views (`external_data_source_dismissals`, `opdb_manufacturer_exceptions`, `opdb_vocabulary_settled`) remain.
