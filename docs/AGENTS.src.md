...
- **Retiring a finding permanently** — a one-off finding gets a dismissal (appended in `bridge.sql` with a date and note; browse `external_data_source_dismissals`); a settled vocabulary value or an adjudicated maker pairing gets the settled/exceptions list (`opdb_vocabulary_settled` in `opdb.sql`, the manufacturer exceptions in `identity.sql`, browsable as `opdb_manufacturer_exceptions`) — a dismissal covers one finding, those cover the value or pairing.
