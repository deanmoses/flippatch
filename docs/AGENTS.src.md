...
The files under `external_data_sources/` are internals, split for editing: `identity.sql` holds both sources' decodes and the one shared model-matching ladder, classification and replay (run once over a unified listing shape — IPDB rows simply carry no group leg), and `ipdb.sql` / `opdb.sql` hold what each source alone has (corporate entities and credits and specialties; titles and manufacturers and vocabulary) plus their findings. A session editing one of them can read just that file (`PREFIX=identity`, or the source name, e.g. `PREFIX=opdb`) — that path loads the live SQL, uncached, which is the point when editing it.
