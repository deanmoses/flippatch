...
- **IPDB note text** — `flippatch/scripts/quote_verify/verify_quotes.py`, `ipdb_notes_text(...)` (via `_Sources.free_text_for`) assembles the editor-authored machine prose — Notable Features, Toys, Notes and Marketing Slogans, each under the IPDB page's own `Label:` — for one IPDB id from the `ipdb_machines` DuckDB table. That prose is the per-record payload the model judges; the structured columns (Manufacturer, Type, Players, Theme, year) are deterministic data, joined or filtered directly in SQL rather than read by the model — _except_ the candidate targets' columns fed in for relational disambiguation.
