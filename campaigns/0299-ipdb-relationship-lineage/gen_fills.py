#!/usr/bin/env python3
"""Emit the sweep-fill patch: the vetted green tier of the 0299 sweep run.

One entry per model from ``lineage.sql``'s ``lineage_fill_patch_rows`` -- the
vetted fill-edge table minus any edge the catalog already carries, so a re-run
after the apply legitimately emits nothing and says so. The vet corrections and
their rationale live beside the table in ``lineage.sql``; the fill ledger there
ties every green sweep row to this table or a named deferral, both directions.

``license_status`` is ``unknown`` on every emitted member; the analysis's
``fill_license_not_unknown`` check pins that against results.json rather than
letting this hardcode drift. Quotes were each verified verbatim before being
recorded, and ``make verify-quote-verbatim`` re-checks the emitted patch.

Run from the flippatch repo root::

    uv run python3 campaigns/0299-ipdb-relationship-lineage/gen_fills.py
    make validate
"""
...
PATCH_PATH = pk.PATCHES_DIR / "0300-conversion-lineage-sweep.yaml"
LINEAGE_SQL = HERE / "lineage.sql"

DESCRIPTION = "Conversion lineage for 51 machines, the 0299 sweep's vetted green tier."


def main() -> int:
    rows = pk.read_view(LINEAGE_SQL, "lineage_fill_patch_rows", prefix="lineage")
    if not rows:
        raise SystemExit(
            "lineage_fill_patch_rows is empty — every vetted fill edge is already in the catalog"
        )
    entries: list[str] = []
