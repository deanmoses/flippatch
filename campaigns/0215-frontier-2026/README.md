...
- **✅ 0226, 0228 · [arabian-nights](model-families/arabian-nights)** (Pedretti) — Tales of the Arabian Nights 30th Anniversary + Legacy Edition **+ 1 older**: TOTAN (Williams, 1996). 0228 is the [document-cites](RULEBOOK.md#document-cites) pilot: the Williams-era documents 0226 couldn't cite.
...
- **✅ 0231 · [p3-modules](model-families/p3-modules)** (Multimorphic) — Dungeon Crawler Carl, Ender's Game
- **✅ 0232 · [yukon-yeti](model-families/yukon-yeti)** (Turner) — Yukon Yeti **+ the whole maker**: Merlin's Arcade (Arcade + Legendary), Ninja Eclipse (Arcade + First). Renames yukon-yeti to (Legendary Edition) and the corporate entity to Turner Logic, LLC (Boerne TX); Ninja Eclipse's Arcade Edition was never produced. Live-but-unlinked maker URLs and the Wayback gzip trap are the promoted learnings — see [RULEBOOK.md](RULEBOOK.md).
- **✅ 0233 + 0235 · [ramps-pinball](model-families/ramps-pinball)** (Ramp's) — Monster League Hockey **+ 2 siblings**: Road Trip, Little Shop of Horrors. 0235 is the opdb-attributed Road Trip month retract (see RULEBOOK → Asserting claims). Snapshot validation pending.
- **✅ 0234 · [vector-pinball](model-families/vector-pinball)** (Vector Pinball) — not on 0215's original list; added 2026-08-13. Holden Heritage + Peter Brock KOTM (Torana, VK Commodore) **+ 1 older**: Eight Ball Fury (2024), **+ 2 created**: Ned Kelly, Lost in Space. The maker publishes no PDFs — its rules/features/specs pages are the spec sheets. Person-creates-for-credits is the promoted learning — see [RULEBOOK.md](RULEBOOK.md#asserting-claims).
- **✅ 0236 · [for-amusement-only-games](model-families/for-amusement-only-games)** (For Amusement Only Games) — no 2026 model; the maker's whole recent-year catalog: Drained **+ 5 siblings**: Ranger in the Ruins, Quest for Glory, Silver Falls, Flipper Foxtrot Rhythm Explosion, Drained Bite-Sized, **+ 1 to create** (user, 2026-08-13): Steelbound (announced 2025). All Multimorphic P3 titles (third-party developer).
...
- **✅ 0271-0274 · [big-lebowski](model-families/big-lebowski)** (Dutch Pinball) — not on 0215's original list; added 2026-08-19 after a Pinside new-games sweep. The Big Lebowski (Jesus Edition) created and enriched **+ 2 older**: the 2015 first model and the 2019 second model. 0273 gathers all three under one `the-big-lebowski` Title and 0274 deletes the Title that orphans — **the move and the delete cannot share a patch** (a delete's referrer check reads live DB state; the 0264/0265 precedent). 0272 moves the corporate entity from Reuver to Herkenbosch. The **Pokémon Home Edition** found in the same sweep is **held** until Stern publishes something first-party — research recorded in [Pokemon.md](model-families/pokemon/Pokemon.md#home-edition--held-2026-08-19). The first model's year is contested (catalog 2015, IPDB and Pinside 2016) and left for the user. `verify-quote-support` still owed for all four (API credits exhausted).
...
**Complete the family's siblings.** Each per-family campaign fleshes out missing data for the whole family, not just its 2026 member(s) — a family's primary documents mostly describe the older machines too, so it is context-efficient to do at once.
...
4. **Emit from a `gen.py`**, facts inline, via patchkit ([DataPatchKit.md](../../../flipcommons/docs/DataPatchKit.md)) — never hand-roll YAML escaping. Record user decisions in the family notes. The per-family generators were deleted once hand revision made them stale; see [RULEBOOK.md](RULEBOOK.md#patch-generator).
