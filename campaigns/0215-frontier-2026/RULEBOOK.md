...
- **A maker document can hide under its internal dev codename.** Pokémon's feature matrix lives at `wp.sternpinball.com/…/PANTS-Matrix-added-©_021226.pdf` — "PANTS" was the game's development codename (Kineticist records it), so no title-word search or site link finds it; it surfaced via web search after journalism named the codename. When a maker's expected document class (Stern always publishes a matrix) is missing under the title's name, hunt the codename before declaring it unpublished — and keep any URL-encoded characters (`%C2%A9`) exactly as cached in the cite ref.
...
**Delete every `note:` the citation already carries (user, 2026-08-13; sharpened same day: when in doubt, delete).** The golden rule is DataPatchAuthoring.md → `note:`: if you can delete the note and the cite still carries the change, delete it. Mapping a maker's "Cabinet Artist" to the art role, or "Creative Director" to design, needs no note — it is obvious from the quote and the value. The 0220-era generators (and 0231's first emit) over-noted heavily; don't copy that habit from the templates. 0237's surviving set is the calibration: mark notes (they are the evidence record for a quote-less mark cite), a genuine classification inference (SPIKE 3 → solid-state, cabinet-from-dimensions), and a shared-document explanation — everything else the quotes carry. The editorial lint now also rejects the old templates' locator and note phrasings ("page 1 of 1", "no printed folio", "maps to", "sought", "at authoring time", "no source") — trust the current lint over any pre-0237 generator's wording.
...
**The model date fields are now `production_year` / `production_month`.** `year:` and `month:` on a claim are rejected outright by `schema/patch.schema.json` (there is no legacy alias, so a patch carrying them would fail the apply); a design/release-to-production date goes in `project_year` / `project_month` instead. Every patch through 0215 uses the old names, so don't copy a date field off an older family's YAML. A *source node's* `year`/`month` is unaffected — that is the cited work's publication date.
...
**One design, one Title — and a Title move cannot share a patch with the delete it orphans.** A re-issue, a revision or a trim edition is the same game design, so it belongs under the base Title however the source databases file it (DomainModel: a Title is the canonical identity of a game design regardless of editions or manufacturers). IPDB mints a fresh row per re-issue and OPDB a fresh game id per home edition, and both artifacts have leaked into the seed as standalone Titles — The Big Lebowski's 2019 "Second Model" was one, fixed by 0273. Emptying a Title then means retiring it, and **a delete's referrer check reads live database state**, so the move and the `delete: true` must be two patches applied in order (the 0264/0265 Harvest Moon precedent says so in a comment; 0273 + 0274 is the second case).

**A patch supports the new state — never the old one (user, 2026-08-19).** A changeset note that narrates what a value used to be, or names the address/date/name being superseded, is a claim about the past dressed as provenance: the cite already carries what the value now is, and the superseded value is already in the claim history. 0272's Dutch Pinball move is the worked case — the note recounting Reuver was deleted and the entry reads correctly with the quote alone.

**A self-evident value takes no note either (user, 2026-08-19).** `location_type: city` on a location restates what the record already is — the app derives the value from the country ancestor's `divisions` and won't let the editor's callers supply one — so there is nothing a note could add. The editorial lint's `note-required` rule now carries a narrow exemption for exactly this, `SELF_EVIDENT_FIELDS` in `lint_patches.py`, keyed (entity type → field). Keep that list short: an ordinary sourced fact still needs a cite or a note, and one ordinary field alongside an exempt one puts the whole entry back under the rule.

**A create of a plain-fact entity takes no cite (user, 2026-08-19).** Citing a machine's press coverage to establish that a town exists is not what a citation is for. 0272 creates `location.netherlands/herkenbosch` bare; the evidence belongs on the entry that says the *company* is there. (0225's location creates carry cites — don't copy them.)

**To change a resolved value, assert it — don't retract (Provenance.md → Superseding).** "A new claim from the same source for the same claim key deactivates the old one", so a patch attributed to the actor that holds the winning claim simply supersedes it, and a lower-priority source's disagreeing copy stays put at rank 2 doing nothing. `retract:` is for **removing** a value, and its documented side effect — letting a lower-priority copy resurface — is the opposite of what a correction wants. 0276 is the worked case: the Big Lebowski first model moved from 2015-05 to 2016-04 with two plain asserts, leaving OPDB's 2015 harmlessly outranked.
