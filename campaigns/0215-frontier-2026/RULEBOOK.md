...
- **A maker document that exists only as images inside an outlet's article is still that maker's document — and still citable.** Extract the image URLs from the article's raw HTML blob (`web_cache.py get` names it; Next.js sites bury them in escaped JSON, WordPress in `<img srcset>`), fetch the full-size images, transcribe, and file the transcription with `web_import.py --text-file` — the quotes then gate as ordinary web text. Cite the image URLs with a locator naming the true publisher ("front sheet of HEXA's product specification sheet, reproduced as an image in Pinball News's reveal article"). A document cite (`<publisher>:<slug>`) is doctrinally cleaner but stays blocked on the two tooling gaps in [Document cites](#document-cites) — web-citing the hosting outlet's copy is the ruled interim pattern (user, 2026-08-12, musketeers).
