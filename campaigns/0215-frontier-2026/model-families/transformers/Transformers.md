...
**Generated and snapshot-validated 2026-08-08; unshipped, awaiting user commit/push.** Patch number **0220** claimed (next free after 0219). One patch for the whole family — Houdini's 0216/0219 split was forced by the 0218 vocab's numbering, which 0220 already follows, so vocabulary and assignments ride one file (vocab entries first, per DataPatchKit.md). 68 entries; all gates green: `make validate` clean, `make verify-quotes` **86 verified / 0 failed / 10 SKIP-PDF** (the matrix + manual transcriptions, author-checked), `make lint`/`typecheck`/`test` clean, and the patch applied cleanly on the standing snapshot (`db.prod.patch-0214.2026-08-03.sqlite3` → migrate → ingest all six pending patches) with fields, features, credits, tag, production_quantity, `variant_of` and the new vocabulary all resolving as intended (spot-checked through `make analyze`, incl. `patch_entry_cites`: mark cites land quote-less with locators, quoted cites carry their spans).
