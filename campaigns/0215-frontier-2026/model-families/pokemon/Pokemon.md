...
## Home Edition — held (2026-08-19)

A **fourth Pokémon model exists and is not in the catalog**: Stern's home edition, which appeared on Costco sales floors on 2026-08-12 at $4,999 with no announcement from either company. The user's ruling (2026-08-19) is to **hold it** until Stern publishes something first-party, because the name cannot be settled from citable evidence:

- **Kineticist** (rooted, citable) calls it "Pokémon Home Edition" throughout — `https://www.kineticist.com/news/pokemon-home-edition-costco`, 2026-08-13.
- **The Costco SKU title is "Stern Pinball Pokémon Home Edition Plus Pinball Machine"** (item #1985364), and Stern's own 2025 manuals name the current line that way — `JP-Home-Manual-Rev-B.pdf` is titled "Jurassic Park Home Edition Plus Manual" and `Star-Wars-Home-Edition-Plus-Manual_web.pdf` likewise, both linked from `https://www.sternpinball.com/manuals`. So "Home Edition Plus" is probably the real SKU — but the only carriers are Costco (bot-walled, no citation root) and a retail aggregator, neither citable.

**Stern has published nothing** as of 2026-08-19: `sternpinball.com/games` does not list it, `/game/pokemon` links only Pro/Premium/Limited Edition, `/game/pokemon/home-edition` and `/game/pokemon/home` both 404, `/manuals` has no Pokémon home-edition manual, and the newest post on `/news` is the 2026-08-04 Stern of the Union. Watch `/manuals` — a `Pokemon_Home_*.pdf` there settles the name, the specs and the model number in one fetch.

**The research is done, so a later session need not redo it.** What Kineticist's article supports, all of it quotable: a **10-inch LCD** ("The display is a 10-inch LCD instead of the 18.5-inch on the full-size games"), **two speakers standard**, a **commercial grade wood playfield** (Stern's own retail-box wording, quoted in the article), Pikachu as a **static sculpt** as on the Pro, Meowth on a **spring mechanism that raises and lowers him**, **all-plastic ramps** replacing wireforms, banks of individual targets become **paddle targets**, the left Poké Ball reduced to a flat plastic piece, no Town Scoop, and an **up post** holding the ball under the reworked Squirtle ramp. Costco's listing adds 1–4 players and Insider Connected — uncitable.

Two cached images from the article, both fetched 2026-08-19: `https://media.kineticist.com/c4a17d46-2bc8-4690-9c12-16808a0e2486.jpg` (playfield with the retail box alongside) and `https://media.kineticist.com/a742767b-5a5a-4ee8-9ada-1494b60c596f.jpg` (the machine on the sales floor). **The box's edition name is not legible** at the resolution served — crop-and-upscale of the box panel reads the feature callouts but no edition wording, so the RULEBOOK's on-machine-print route does not settle the name either. Neither image has a transcription filed, so neither is citable yet.

When it is created: the four existing Stern Home Editions each sit under their own Title (an OPDB artifact — OPDB mints a separate game id per home edition), which conflicts with DomainModel's "a Title is the game design regardless of edition" and with how 0215 filed the 2026 editions. The user was asked and deferred the question along with the model. All four also carry the `home-use` tag and no `variant_of` edge.
