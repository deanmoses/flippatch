...
- **Never name a reference database or a glossary in a description.** IPDB, OPDB and "glossary" are out — the fact goes in the prose, the attribution in the `cite:`. Enforced by the `description-source-name` lint over descriptions only (a `note:` may still name a source, as 0242 does), from patch 0260 on. Naming any *other* site stays legal when the prose hyperlinks it, as 0252 does for bingo.cdyn.com.
